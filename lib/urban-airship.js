...
//Platform constants
UrbanAirship.prototype.ANDROID = "android";
UrbanAirship.prototype.IOS = "ios";

/**
 * Gets Urban Airship constant string for device Id's depending on platform
 *
 * @params platform: Use ANDROID and IOS constants.
 * @return Urban Airship constant string for device Id's
 */
UrbanAirship.prototype.getDeviceIdConstant = function(platform) {
	if (platform === this.ANDROID) return 'apids';
	else return "device_tokens";
}
 ...
 */
UrbanAirship.prototype.getDeviceCounts = function(platform, callback) {
	this._auth = new Buffer(this._key + ":" + this._master, "utf8").toString("base64");

	this._transport("/api/" + this.getDeviceIdConstant(platform) + "/count/", "GET", function(error, response_data) {
 ...
 *
 * @params
 *	platform - Use ANDROID and IOS constants.
 *	device_id - The device identifier
 *	data - The JSON payload (optional)
 *	callback
 */
UrbanAirship.prototype.registerDevice = function(platform, device_id, data, callback) {
	this._auth = new Buffer(this._key + ":" + this._secret, "utf8").toString("base64");

	var path = "/api/" + this.getDeviceIdConstant(platform) + "/" + device_id;
 ...
 *	callback
 */
UrbanAirship.prototype.unregisterDevice = function(platform, device_id, callback) {
	this._auth = new Buffer(this._key + ":" + this._secret, "utf8").toString("base64");
	this._transport("/api/" + this.getDiviceIdConstant(platform) + "/" + device_id, "DELETE", callback);
}
